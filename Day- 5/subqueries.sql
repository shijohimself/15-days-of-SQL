...
SELECT name FROM movies
WHERE score = (SELECT MAX(score) FROM movies WHERE
...
SELECT * FROM movies
WHERE (genre,score) IN (SELECT genre,max(score) AS 'score'
FROM movies
WHERE votes > 25000
GROUP BY genre);

# find the higehst grossing movie of top 5 actor/director combo in terms of 
# total gross income
WITH top_duos AS (SELECT star,director,MAX(gross) as 'gross'
FROM movies
